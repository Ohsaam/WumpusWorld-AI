...
#title은 이 뷰가 띄우는 tkinter 창의 제목
    def __init__(self, xy_env, max_thing_width, max_thing_height,title):
   ...
   # CANVAS_HEIGHT는 캔버스의 크기를 나타내며, 
        ...
        self.NEWCANVAS_WIDTH = max_thing_width * self.xy_env.width
        self.NEWCANVAS_HEIGHT = max_thing_height * self.xy_env.height
...
#loc는 튜플로, 첫 번째 원소는 가로 좌표, 두 번째 원소는 세로 좌표
    ...
    def animate_thing(self, thing, loc1, loc2):
        ...
        def moveme():
            nonlocal orig_x, orig_y, FRAMES, delta_x, delta_y
...
#이전에 그려진 모든 객체를 지우고, xy_env에서 받아온 thing과 그 위치(loc)를 사용하여 각 객체를 화면에 그림
    ...
    def getPercept(self, gack):
        self.stench=gack.stench
        self.breeze=gack.breeze
        self.glitter=gack.glitter
        self.bump=gack.bump
        self.scream=gack.scream
    ...
    def draw(self, thing, loc):
        ...
        # Draw on main window
        if basename not in ['wumpus.gif', 'pit.gif', 'gold.gif']:
            self.canvas.create_image(x + self.square_width // 2,
                y + self.square_height // 2,
                image=self.image_cache[filename], tags='thing')

        # Draw on new window if it's a wumpus, pit, or gold
        if basename in ['wumpus.gif', 'pit.gif', 'gold.gif']:
            self.newCanvas.create_image(x + self.square_width // 2,
                y + self.square_height // 2,
                image=self.image_cache[filename], tags='thing')
        # print(wumpus.stench)
        # percept_str = "Stench: {}, Breeze: {}, Glitter: {}, Bump: {}, Scream: {}".format(
        # wumpus.WumpusEnvironment.stench, wumpus.WumpusEnvironment.breeze, wumpus.WumpusEnvironment.glitter, wumpus.WumpusEnvironment.bump, wumpus.WumpusEnvironment.scream)
        # print("Percept: {}".format(percept_str))


    def run_until(self, steps=1000):

      #  percept_str = f"{wumpus.WumpusEnvironment.stench}, {wumpus.WumpusEnvironment.breeze}, {wumpus.WumpusEnvironment.glitter}, {wumpus.WumpusEnvironment.bump}, {wumpus.WumpusEnvironment.scream}"
       # self.progress.set(f"Percept: {percept_str} ({self.total_steps} steps)")
    ...
    def createNewWindow(self):
        self.newWindow = tk.Toplevel(self.root_window, takefocus=True)
        self.newWindow.grid()
        self.newWindow.title("Solution")
        self.newWindow.config(background='white')

        self.newCanvas = tk.Canvas(self.newWindow,
                width=self.NEWCANVAS_WIDTH, height=self.NEWCANVAS_HEIGHT, bg='black')
        self.newCanvas.grid(row=0,column=0,columnspan=6,sticky='we')
...
#ttk.Label, ttk.Entry, ttk.Button, ttk.Checkbutton를 사용하여 GUI의 버튼, 체크박스, 라벨 등을 생성하고 이를 배치
        ...
        self.canvas = tk.Canvas(self.root_window,
            width=self.CANVAS_WIDTH, height=self.CANVAS_HEIGHT, bg='white', highlightthickness=2, highlightbackground='gray')

        self.canvas.grid(row=0,column=0,columnspan=6,sticky='we') #self.canvas: GUI에서 캔버스를 생성하고 이를 저장

# Draw grid lines
        for i in range(self.xy_env.width + 1):
            x = i * self.square_width
            self.canvas.create_line(x, 0, x, self.CANVAS_HEIGHT, width=2, fill='gray')

        for i in range(self.xy_env.height + 1):
            y = i * self.square_height
            self.canvas.create_line(0, y, self.CANVAS_WIDTH, y, width=2, fill='gray')
